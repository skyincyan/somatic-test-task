...
vcf_for_bed = pd.DataFrame({'chrom': vcf['CHROM'].map(lambda x: x[3:].replace('X',' 23').replace('Y', '24')).astype(int), 
'start': vcf['POS'], 'end': vcf['POS']+vcf['ALT'].map(len), 'name': vcf.index})    
